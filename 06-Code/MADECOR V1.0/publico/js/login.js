...
document.getElementById("login-form").addEventListener("submit", async (e) => {
    ...
    try {
        ...
        // Consola para verificar los permisos recibidos
        console.log("Permisos recibidos:", data.modulos || []);

        // Almacenar información del usuario y los módulos en localStorage
        localStorage.setItem("user", JSON.stringify({
            name: data.user.usuario_nombre,
            email: data.user.usuario_email,
            id: data.user.usuario_id,
            type: data.user.usuario_tipo,
            modulos: data.modulos || [], // Aquí guardamos los módulos de permisos
        }));
        ...
        setTimeout(() => {
            if (data.user.usuario_tipo === "trabajador") {
                window.location.href = "html/inicio.html"; // Página para trabajadores
            } else {
                window.location.href = "index.html"; // Página para clientes
            }
        }, 2000);
    } catch (error) {
