...
document.addEventListener('DOMContentLoaded', () => {
    ...
    formProducto.addEventListener('submit', function(event) {
        ...
        fetch('http://localhost:3000/madecor/productos', {
            method: 'POST',
            headers: {
    ...
    function obtenerProductos(query = '') {
        fetch(`http://localhost:3000/madecor/productos?q=${query}`)
            .then(response => response.json())
            .then(data => {
    ...
    window.editarProducto = function (id) {
        ...
        fetch(`http://localhost:3000/madecor/productos/${id}`)
            .then(response => {
                if (!response.ok) {
    ...
    window.cambiarEstadoProducto = function (id, estadoActual) {
        ...
        fetch(`http://localhost:3000/madecor/productos/${id}/estado`, {
            method: 'PATCH',
