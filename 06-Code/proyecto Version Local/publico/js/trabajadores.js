...
cargarPermisos('trabajadorPermiso'); // Para creación
    ...
    formTrabajador.addEventListener('submit', function(event) {
        ...
        fetch('http://localhost:3000/madecor/trabajadores', {
            method: 'POST',
            headers: {
    ...
    function obtenerTrabajadores(query = '') {
        fetch(`http://localhost:3000/madecor/trabajadores?q=${query}`) // Pasar el parámetro q
            .then(response => response.json())
            .then(data => {
...
window.editarTrabajador = function (cedula) {
    ...
    fetch(`http://localhost:3000/madecor/trabajadores/${cedula}`)
        .then(response => {
...
window.cambiarEstadoTrabajador = function (cedula, estadoActual) {
    ...
    fetch(`http://localhost:3000/madecor/trabajadores/${cedula}/estado`, {
        method: 'PATCH',
