...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function cargarProductos() {
    const response = await fetch("http://localhost:3000/madecor/productos"); // Cambia a tu endpoint real
  ...
  document.getElementById("formVenta").addEventListener("submit", async e => {
    ...
    const response = await fetch("http://localhost:3000/madecor/ventas", {
      method: "POST",
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    formVenta.addEventListener('submit', function(event) {
      ...
      fetch('http://localhost:3000/madecor/ventas', {
          method: 'POST',
          headers: {
...
function obtenerVentas(query = '') {
  let url = `http://localhost:3000/madecor/ventas`;
  if (query) {
