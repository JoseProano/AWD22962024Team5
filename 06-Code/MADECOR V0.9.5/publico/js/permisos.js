...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function obtenerPermisos(query = '') {
        fetch(`http://localhost:3015/madecor/permisos?q=${query}`) // Pasar query como parámetro
    ...
    window.editarPermiso = function (id) {
        ...
        fetch(`http://localhost:3015/madecor/permisos/${id}`)
            .then(response => response.json())
            .then(data => {
...
formEditarPermiso.addEventListener('submit', function (event) {
    ...
    fetch(`http://localhost:3015/madecor/permisos/${permisoId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    .catch(err => {
      ...
      mostrarMensaje('Error al editar permiso.', 'error');
        ...
        window.cambiarEstado = function (id, estadoActual) {
            ...
            fetch(`http://localhost:3015/madecor/permisos/${id}/estado`, {
              method: 'PATCH',
