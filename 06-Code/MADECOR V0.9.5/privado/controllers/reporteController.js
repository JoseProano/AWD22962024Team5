...
// Reporte de Pedidos
const obtenerReportePedidos = (req, res) => {
  const query = req.query.q || '';
  let sql = `
    SELECT p.*, 
           u_cliente.usuario_nombre AS cliente_nombre, 
           u_cliente.usuario_apellido AS cliente_apellido
    FROM pedido p
    JOIN cliente c ON p.pedido_cliente_id = c.cliente_id
    JOIN usuario u_cliente ON c.cliente_id = u_cliente.usuario_id
  `;
  ...
  connection.query(sql, query ? queryParams : [], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ message: 'Error al obtener reporte de pedidos' });
    }
    res.status(200).json(results);
  });
};
...
const obtenerReporteDetallesPedidos = (req, res) => {
  const query = req.query.q || '';
  let sql = `
    SELECT dp.*, 
           pr.producto_nombre, 
           p.pedido_fecha_creacion,
           u_cliente.usuario_nombre AS cliente_nombre,
           u_cliente.usuario_apellido AS cliente_apellido
    FROM detalle_pedido dp
    JOIN producto pr ON dp.detalle_pedido_producto_id = pr.producto_id
    JOIN pedido p ON dp.detalle_pedido_pedido_id = p.pedido_id
    JOIN cliente c ON p.pedido_cliente_id = c.cliente_id
    JOIN usuario u_cliente ON c.cliente_id = u_cliente.usuario_id
  `;
  ...
  connection.query(sql, query ? queryParams : [], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ message: 'Error al obtener detalles de pedidos' });
    }
    res.status(200).json(results);
  });
};
...
const obtenerReporteClientes = (req, res) => {
  const query = req.query.q || '';
  let sql = `
    SELECT u.*, c.cliente_pais, c.cliente_direccion 
    FROM cliente c
...
const obtenerReporteTrabajadores = (req, res) => {
  const query = req.query.q || '';
  let sql = `
    SELECT u.*, t.trabajador_perfil, p.permiso_nombre 
    FROM trabajador t
