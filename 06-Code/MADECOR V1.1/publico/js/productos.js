...
document.addEventListener('DOMContentLoaded', () => {
    ...
    formProducto.addEventListener('submit', function(event) {
        ...
        const productoData = {
            ...
            producto_imagen: document.getElementById('productoImagen').files[0],
        };

        fetch('/madecor/productos', {
            method: 'POST',
            headers: {
    ...
    function obtenerProductos(query = '') {
        fetch(`/madecor/productos?q=${query}`)
            .then(response => response.json())
            .then(data => {
    ...
    window.cambiarEstadoProducto = function (id, estadoActual) {
        ...
        fetch(`/madecor/productos/${id}/estado`, {
            method: 'PATCH',
